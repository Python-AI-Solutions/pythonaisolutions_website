...
test.describe('About Page', () => {
  ...
  test('should display team members', async ({ page }) => {
    ...
    const teamSection = page.locator('section').filter({ hasText: /team|members|people/i });
  ...
  test('should have working team member modals', async ({ page }) => {
    ...
    const clickableElements = teamSection.first().locator('button, [role="button"], a[href="#"]');
  ...
  test('should display team member images', async ({ page }) => {
    ...
    const images = page.locator('img').filter({ hasNot: page.locator('[src*="logo"]') });
