...
test.describe('Home Page', () => {
  ...
  test('should display company logo and mission statement', async ({ page }) => {
    // Check for Python AI Solutions logo
    const logo = page.locator('img[alt*="Python AI Solutions Logo"], img[alt*="Logo"]')
    await expect(logo.first()).toBeVisible()

    // Check for mission statement paragraph
    const missionText = page.locator('text=We are committed to the democratization of AI')
  ...
  test('should display clients section with heading', async ({ page }) => {
    ...
    const clientsHeading = page.locator('text=Providing trusted services globally')
  ...
  test('should display testimonials carousel', async ({ page }) => {
    ...
    const navDots = page.locator('button[class*="rounded-full"][class*="h-2"], button[class*="rounded-full"][class*="h-3"]')
    ...
    const navigationButtons = page.locator('button').filter({ has: page.locator('svg') })
  ...
  test('should display "Read more" buttons in testimonials', async ({ page }) => {
  ...
  test('should open testimonial modal when "Read more" is clicked', async ({ page }) => {
  ...
  test('should display "Provide a testimonial" button', async ({ page }) => {
    ...
    const testimonialButton = page.locator('button:has-text("Provide a testimonial"), a:has-text("Provide a testimonial")').first()
  ...
  test('should navigate to testimonial page', async ({ page }) => {
    ...
    const testimonialButton = page.locator('a:has-text("Provide a testimonial")').first()
  ...
  test('should display services section', async ({ page }) => {
    ...
    const servicesHeading = page.locator('text=We help you harness the power of artificial intelligence')
  ...
  test('should display ContactSection with proper content', async ({ page }) => {
    // Scroll to contact section
    await page.locator('body').press('End')
    
    // Check for "Let's Work Together" heading
    const workTogetherHeading = page.locator('h2:has-text("Let\'s Work Together")')
    const headingExists = await workTogetherHeading.isVisible().catch(() => false)
    
    if (headingExists) {
      await expect(workTogetherHeading).toBeVisible()
      
      // Check for contact section description paragraphs
      const contactDescription1 = page.locator('text=We\'re always looking for new opportunities')
      await expect(contactDescription1).toBeVisible()
      
      const contactDescription2 = page.locator('text=Please get in touch to learn more')
      await expect(contactDescription2).toBeVisible()
      
      // Check for "Contact Us" button in contact section - try different variations
      let contactUsButton = page.locator('a[href="/contact"]:has-text("Contact Us")')
      let buttonExists = await contactUsButton.isVisible().catch(() => false)
      
      if (!buttonExists) {
        // Try with different casing
        contactUsButton = page.locator('a[href="/contact"]:has-text("Contact us")')
